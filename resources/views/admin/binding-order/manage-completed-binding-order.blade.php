                ...
                <table class="table" id="enquiry-table">
                  <thead>
                    <tr>
                      ...
                      <th width="170px">Completed Date & Time</th>
                      <th>Order Number</th>
                      <th>Name & Email</th>
                      <th>Phone Number</th>
                      <th>Address</th>
                  ...
                  <tbody>
                    @if (isset($completed_binding_orders) && count($completed_binding_orders)>0)
                    @foreach ($completed_binding_orders as $binding_order)
                    <tr>
                      <td>#{{ $loop->iteration }}</td>
                      <td>{{ $binding_order->created_at }}</td>
                      <td>{{ $binding_order->order_completed_date }}</td>
                      ...
                      <td>{{ $binding_order->binding_type->name ?? '-' }}-<span>{{ $binding_order->colour->name ?? '-' }}</span></td>
                      <td>
                      ...
                      <td>
                        ...
                        {{ $binding_order->paper_grade->name ?? '-' }} <br>
                      ...
                      <td>
						...
						<td><a href="{{ route('admin.generate-binding-pdf',$binding_order->id) }}" target="_blank">Preview</a></td>
...
<script type="text/javascript">
  ...
  function deleteConfirmation(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url:`{{ URL::to('admin/manage-binding-order/${id}') }}`,
          type:"DELETE",
          dataType:"json",
  ...
  $(document).ready( function () {
    ...
    $(document).on('click','.view-embossing-detail',function(event) {
        ...
        $.ajax({
          url:`{{ URL::to('admin/fetch-binding-embossing-detail/${order_id}') }}`,
          type:"get",
          dataType:"json",
          success:function(result) {
            if(result.success) {
    ...
    $(document).on('click','.view-extra-detail',function(event) {
        let order_id = $(this).attr('order_id');
        $.ajax({
          url:`{{ URL::to('admin/fetch-binding-extra-detail/${order_id}') }}`,
          type:"get",
          dataType:"json",
          success:function(result) {
            if(result.success) {
