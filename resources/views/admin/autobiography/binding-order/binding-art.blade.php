    ...
    <div class="invoice-blk">
      <table>
        <tbody>
			<tr>
            <td>
              <button type="button" onclick="window.print()" class="noprint print-button">Print</button>
            </td>	  
          </tr>
          <tr>
            <td>
              <h2 style="text-align: center;margin: 0 0 10px;font-size: 25px;">{{ $binding_order->autobiography_binding_type->name ?? '' }}</h2>
            </td>	  
          </tr>
          <tr>
            <td>
              <div class="front-image-blk">
                ...
                <div class="book-front-content-blk">
                  ...
                  @if (isset($binding_order->autobiography_university))
                    @if ($binding_order->embossing_colour == 'silver')
                      <img src="{{ URL::asset('storage/'.$binding_order->autobiography_university->logo_silver) }}" class="book-cover-logo">
                    @endif
                    @if ($binding_order->embossing_colour == 'black')
                      <img src="{{ URL::asset('storage/'.$binding_order->autobiography_university->logo_black) }}" class="book-cover-logo">
                    @endif
                    @if ($binding_order->embossing_colour == 'golden')
                      <img src="{{ URL::asset('storage/'.$binding_order->autobiography_university->logo_gold) }}" class="book-cover-logo">                      
                    @endif
                  @endif
    ...
    <script>
      ...
      function printPDF() {
        window.html2canvas = html2canvas;
          ...
          printDoc.html(document.getElementById('pdf'), {
            callback: function (printDoc) {
              printDoc.save('pdf-art');
            }
          });
      }
    </script>
