                  ...
                  <tbody>
                      @if (isset($universities) && count($universities)>0)
                          @foreach ($universities as $university)
                            ...
                            <td>{{ $university->autobiography_university_country->name }}</td>
...
<script type="text/javascript">
  ...
  function deleteConfirmation(id){
      Swal.fire({
          ...
          if (result.isConfirmed) {
              $.ajax({
                  url:`{{ URL::to('admin/manage-bio-university/${id}') }}`,
                  type:"DELETE",
                  dataType:"json",
  ...
  $(document).ready( function () {
    ...
    $(document).on('click','#add-university',function(event){
        $.ajax({
            url:"{{ URL::to('admin/manage-bio-university/create') }}",
            type:"GET",
            dataType:"json",
            success:function(result) {
                if (result.success) {
    ...
    $(document).on("click","#add-university-btn",function(event){
        ...
        $.ajax({
            url:"{{ URL::to('admin/manage-bio-university') }}",
            type:'POST',
            processData: false,
            contentType: false,
            dataType:'json',
    ...
    $(document).on("click",".edit-university",function(event){
      ...
      $.ajax({
          url:`{{ URL::to('admin/manage-bio-university/${id}/edit') }}`,
          type:"get",
          dataType:"json",
          success:function(result) {
            if(result.success) {
