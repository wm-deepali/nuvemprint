                  ...
                  <tbody>
                      @if (isset($binding_type_images) && count($binding_type_images)>0)
                          @foreach ($binding_type_images as $binding_type_image)
                            ...
                            <td>{{ $binding_type_image->binding_type->name ?? '-' }}</td>
                            <td>{{ $binding_type_image->colour->name ?? '-' }}</td>
                            <td>
...
<script type="text/javascript">
  ...
  function deleteConfirmation(id){
      Swal.fire({
          ...
          if (result.isConfirmed) {
              $.ajax({
                  url:`{{ URL::to('admin/manage-binding-type-image/${id}') }}`,
                  type:"DELETE",
                  dataType:"json",
  ...
  $(document).ready( function () {
    ...
    $(document).on('change','#binding_type',function(event){
      ...
      $.ajax({
        url:`{{ URL::to('admin/fetch_colour_by_binding_type/${binding_type}') }}`,
        type:"GET",
        dataType:"json",
        success:function(result) {
          if (result.success) {
    ...
    $(document).on('click','#add-binding-type-image',function(event){
        $.ajax({
            url:"{{ URL::to('admin/manage-binding-type-image/create') }}",
            type:"GET",
            dataType:"json",
            success:function(result) {
                if (result.success) {
    ...
    $(document).on("click","#add-binding-type-image-btn",function(event){
        ...
        $.ajax({
            url:"{{ URL::to('admin/manage-binding-type-image') }}",
            type:'POST',
            processData: false,
            contentType: false,
            dataType:'json',
