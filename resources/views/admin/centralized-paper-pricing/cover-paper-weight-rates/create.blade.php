            ...
            $(document).ready(function () {
                ...
                function getDependencyRowHtml(index) {
                    ...
                    html += `
                            <div class="col-md-3">
                                <label>Price</label>
                                <input type="number" name="rows[${index}][price]" class="form-control price-input" step="0.01" min="0" placeholder="Price of SRA3 sheet" required data-index="${index}">
                ...
                $('#paper-weight-form').submit(function (e) {
                    e.preventDefault();
                    $.ajax({
                        url: "{{ route('admin.cover-paper-weight-rates.store') }}",
                        method: "POST",
                        data: $(this).serialize(),
