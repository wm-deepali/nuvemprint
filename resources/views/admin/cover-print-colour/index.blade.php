    ...
    <div class="content-header row">
      ...
      <div class="row breadcrumbs-top">
        ...
        <div class="breadcrumb-wrapper">
          ...
          <li class="breadcrumb-item active">Cover Print Colours</li>
          </ol>
        </div>
        </div>
      </div>
      ...
      <div class="form-group breadcrumb-right">
        <div class="dropdown">
        <a href="javascript:void(0)" class="btn btn-primary btn-sm btn-round" id="cover-add-print-colour">Add</a>
        </div>
      </div>
      </div>
    </div>
    ...
    <div class="content-body">
      ...
      <div class="col-md-12">
        ...
        <div class="card-header">
          <h4 class="card-title">Cover Print Colours</h4>
        </div>
        <div class="card-body">
          ...
          <table class="table" id="print-colour-table">
            ...
            <tbody>
            @foreach($printColours as $printColour)
        <tr>
          <td>{{ $loop->iteration }}</td>
          <td>{{ $printColour->name }}</td>
          <td>
          <ul class="list-inline">
          <li class="list-inline-item">
          <a href="javascript:void(0)" class="btn btn-sm btn-primary btn-sm-custom edit-cover-print-colour"
            data-id="{{ $printColour->id }}">
            <i class="fas fa-pencil-alt"></i>
          </a>
          </li>
          <li class="list-inline-item">
          <a href="javascript:void(0)" onclick="deletePrintColour({{ $printColour->id }})"
            title="Delete">
...
@push('scripts')
  <script>
    ...
    Swal.fire({
      ...
      $.ajax({
        url: `/admin/manage-cover-print-colour/${id}`,
        type: "DELETE",
    ...
    // Load Create Modal
    $('#cover-add-print-colour').on('click', function () {
      $.get("{{ route('admin.manage-cover-print-colour.create') }}", function (response) {
      if (response.success) {
    ...
    // Load Edit Modal
    $(document).on('click', '.edit-cover-print-colour', function () {
      let id = $(this).data('id');
      $.get(`/admin/manage-cover-print-colour/${id}/edit`, function (response) {
      if (response.success) {
        $('#print-colour-modal').html(response.html).modal('show');
      }
      });
    ...
    $(document).on('click', '#add-print-colour-btn', function () {
      ...
      $.ajax({
      url: "{{ route('admin.manage-cover-print-colour.store') }}",
      type: 'POST',
      data: formData,
      contentType: false,
      processData: false,
      success: function (response) {
        if (response.success) {
        Swal.fire('Created!', 'Cover Print Csolour added successfully.', 'success');
    ...
    $(document).on('click', '#update-print-colour-btn', function () {
      $(this).prop('disabled', true);
      let id = $(this).data('id');
      let formData = new FormData(document.getElementById('print-colour-form'));
