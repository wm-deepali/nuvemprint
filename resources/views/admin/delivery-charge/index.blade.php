...
@push('scripts')
  <script>
    ...
    $(document).on('click', '#add-delivery-charge', function () {
      $.get("{{ route('admin.delivery-charges.create') }}", function (result) {
      if (result.success) {
    ...
    $(document).on('click', '.edit-delivery-charge', function () {
      ...
      $.get(`{{ url('admin/delivery-charges') }}/${id}/edit`, function (result) {
      if (result.success) {
        $('#delivery-charge-modal').html(result.html).modal('show');
        ClassicEditorInit();
    ...
    $(document).on('click', '#save-delivery-charge-btn', function () {
      ...
      const id = $(this).data("id");
      const url = id ? `{{ url('admin/delivery-charges') }}/${id}` : `{{ url('admin/delivery-charges') }}`;
