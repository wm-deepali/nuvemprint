...
class CoverPaperWeightRatesController extends Controller
{
    public function create()
    {
        $attribute = Attribute::with('values', 'parents.values')
            ->where('name', 'Cover Paper Weight')  // or ->where('id', X) if you prefer by ID
            ->firstOrFail();

        // dd($attribute->toArray());
        return view('admin.centralized-paper-pricing.cover-paper-weight-rates.create', compact('attribute'));
    }
    ...
    public function edit($attributeId)
    {
        ...
        return view('admin.centralized-paper-pricing.cover-paper-weight-rates.edit', compact(
            'attribute',
            'pricingRules'
        ));
    }
