      ...
      <div class="col-md-12">
        ...
        <div class="card-body">
          ...
          <table class="table table-striped">
            ...
            <tbody>
            <?php $__empty_1 = true; $__currentLoopData = $proofReading; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <tr>
          ...
          <td>
          <div class="d-flex">
          <a class="btn btn-sm btn-info mr-1 text-white edit-proof-reading" data-id="<?php echo e($item->id); ?>">
          Edit
          </a>
...
<?php $__env->startPush('scripts'); ?>
  <script>
    ...
    $(document).on('click', '#add-proof-reading', function () {
      $.get("<?php echo e(route('admin.proof-reading.create')); ?>", function (result) {
      if (result.success) {
    ...
    $(document).on('click', '.edit-proof-reading', function () {
      ...
      $.get(`<?php echo e(url('admin/proof-reading')); ?>/${id}/edit`, function (result) {
      if (result.success) {
        $('#proof-reading-modal').html(result.html).modal('show');
    ...
    $(document).on('click', '#save-proof-reading-btn', function () {
      ...
      const id = $(this).data("id");
      const url = id ? `<?php echo e(url('admin/proof-reading')); ?>/${id}` : `<?php echo e(url('admin/proof-reading')); ?>`;
    ...
    Swal.fire({
      ...
      $.ajax({
        url: `<?php echo e(url('admin/proof-reading')); ?>/${id}`,
        type: 'POST',
