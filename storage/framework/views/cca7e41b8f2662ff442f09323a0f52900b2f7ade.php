...
<div class="col-md-3">
    <label>Price (per 1000 sheets)</label>
    <div class="input-group">
        <input type="number" name="rows[${index}][price]" class="form-control price-input" step="0.01" min="0" placeholder="Price per 1000" required data-index="${index}">
        <div class="input-group-append">
            <span class="input-group-text">£<span class="per-sheet-price" id="per-sheet-${index}">0.00</span></span>
        </div>
    </div>
    <small class="text-muted">Per sheet (auto): right side</small>
</div>
                ...
                $('#paper-weight-form').submit(function (e) {
                    e.preventDefault();
                    $.ajax({
                        url: "<?php echo e(route('admin.paper-weight-rates.store')); ?>",
                        method: "POST",
                        data: $(this).serialize(),
