...
<script>
    ...
    function handleAttributeConditions(selectedAttrId, selectedValueId) {
        // First, reset all affected attributes
        $('.attribute-wrapper').removeClass('d-none');
        $('.attribute-condition-note').remove();
        $('.attribute-values').show();
        $('.attribute-placeholder').remove();
        // Reset all hidden values (in case of toggle between conditions)
        $('.attribute-values [data-value-id]').show();


        // Hide all affected attributes first
        attributeConditions.forEach(condition => {
            const $affectedWrapper = $(`[data-attribute-id="${condition.affected_attribute_id}"]`);
            $affectedWrapper.addClass('d-none');
            $affectedWrapper.find('.attribute-condition-note').remove();
            $affectedWrapper.find('.attribute-values').show();
        });

        // Now show only those that match condition
        attributeConditions.forEach(condition => {
            const isMatch =
                parseInt(condition.parent_attribute_id) === parseInt(selectedAttrId) &&
                parseInt(condition.parent_value_id) === parseInt(selectedValueId);

            const $affectedWrapper = $(`[data-attribute-id="${condition.affected_attribute_id}"]`);

            if (condition.action === 'show') {
                if (isMatch) {
                    $affectedWrapper.removeClass('d-none');
                    $affectedWrapper.find('.attribute-values').show();
                } else {
                    $affectedWrapper.removeClass('d-none');
                    $affectedWrapper.find('.attribute-values').hide();
                    const $parentWrapper = $(`[data-attribute-id="${condition.parent_attribute_id}"]`);
                    const parentName = $parentWrapper.find('label')
                        .clone()
                        .children()
                        .remove()
                        .end()
                    ...
                    // Get selected value's label
                    let parentValueLabel = '';
                    const $selectedBtn = $parentWrapper.find(`[data-value-id="${condition.parent_value_id}"]`);
                    if ($selectedBtn.length) {
                        parentValueLabel = $selectedBtn.text().trim();
                    } else {
                        // Try fallback for dropdown/select
                        parentValueLabel = $parentWrapper.find(`option[data-value-id="${condition.parent_value_id}"]`).text().trim();
                    }

                    // Final note
                    const note = `<p class="attribute-condition-note text-danger mt-2">
    Only available when <strong>${parentName}</strong> is set to <strong>${parentValueLabel}</strong>.
</p>`;

                    $affectedWrapper.append(note);
                }
            }

            if (condition.action === 'hide' && isMatch) {
                if (condition.all_values_affected) {
                    $affectedWrapper.addClass('d-none');
                } else {
                    // Only hide specific values
                    condition.affected_value_ids.forEach(valueId => {
                        $affectedWrapper.find(`[data-value-id="${valueId}"]`).hide();
                    });
                }
            }
    ...
    function getBasePriceFromQuantity(quantity) {
        ...
        quantityPricing.forEach((q) => {
            if (!maxRange || q.quantity_to > maxRange.quantity_to) {
                maxRange = q;
            }
            if (quantity >= q.quantity_from && quantity <= q.quantity_to) {
                matched = q;
            }
        });

        if (matched) {
            return parseFloat(matched.base_price);
        }

        // If quantity exceeds all defined ranges, use highest range's price
        return parseFloat(maxRange?.base_price || 0);
    }


    function getModifierAmount(price, modifier, base) {
        if (modifier === 'multiply') {
            return base * parseFloat(price);
        }
    ...
    // Highlight and select value
    $(document).on('click', '.print-color, .choose-binding, .btn-light', function () {

        const parent = $(this).closest('[class*="color-print"], .paper-type-section');
        parent.find('[data-selected="true"]').removeAttr('data-selected');
    ...
    $('select.custom-select').on('change', function () {
        $(this).find('option').removeAttr('data-selected');
        $(this).find('option:selected').attr('data-selected', true);
        calculateTotalPrice();
    ...
    $('select.custom-select').on('change', function () {
        ...
        const attrId = selectedOption.data('attribute-id');
        const valueId = selectedOption.data('value-id');

        // Deselect all and set selected
        $(this).find('option').removeAttr('data-selected');
        selectedOption.attr('data-selected', true);

        // Fire pricing + condition logic
        calculateTotalPrice();
        handleAttributeConditions(attrId, valueId);
    });


    function extractAttributeId(nameAttr) {
    ...
    document.querySelectorAll('.color-print, .color-print1, .paper-type-section').forEach(section => {
        const options = section.querySelectorAll('.print-color, .choose-binding, .btn');
    ...
    $(document).on('click', '.btn-light', function () {
        ...
        // Update corresponding preview image
        const imageEl = $(`#preview-image-${attrId}`);
    ...
    $(document).on('click', '.print-color, .choose-binding, .btn-light', function () {
        const wrapper = $(this).closest('[data-attribute-id]');
        const attrId = wrapper.data('attribute-id');
        const valueId = $(this).data('value-id');

        // Highlight logic
        wrapper.find('[data-selected]').removeAttr('data-selected');
        $(this).attr('data-selected', true);

        // Call condition check
        handleAttributeConditions(attrId, valueId);
    });
    ...
    document.querySelectorAll('.help-circle').forEach(circle => {
        circle.addEventListener('click', () => {
            ...
            modalTitle.textContent = `${label} Help`;
            modalBody.textContent = helpContent[label] || 'No help content available.';
        });
    });
    ...
    $(document).ready(function () {
        ...
        // Trigger condition check on load
        $(document).ready(function () {
            calculateTotalPrice();
...
<?php /**PATH /home/tpmhelpinghand/public_html/nuvem.tpmhelpinghand.com/resources/views/front/calculator.blade.php ENDPATH**/ ?>